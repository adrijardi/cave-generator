...
class HoverControls(camera: Camera, element: Element, val center: Vector3 = new Vector3()) extends RotateControls {
  ...
  def attach(el: Element) = {
    ...
    el.addEventListener(
      "mousedown",
      (this.onMouseDown _).asInstanceOf[Function[Event, _]]
    )
    el.addEventListener(
      "mousemove",
      (this.onMouseMove _).asInstanceOf[Function[Event, _]],
      false
    )
    el.addEventListener(
      "mouseup",
      (this.onMouseUp _).asInstanceOf[Function[Event, _]],
      false
    )
    el.addEventListener(
      "mousewheel",
      (this.onMouseWheel _).asInstanceOf[Function[Event, _]]
    )
    el.addEventListener(
      "DOMMouseScroll",
      (this.onMouseWheel _).asInstanceOf[Function[Event, _]],
    ...
    el.addEventListener(
      "contextmenu",
      (this.onContextMenu _).asInstanceOf[Function[Event, _]],
      false
    )
